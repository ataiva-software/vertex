package com.ataiva.eden.integration.security
...
class SecurityRegressionTest {
    ...
    @Test
    fun `all endpoints require proper authentication`() = runTest {
    ...
    @Test
    fun `user permissions are enforced correctly`() = runTest {
        ...
        val createResponse = makeRequestWithToken("POST", "$vaultServiceUrl/api/v1/secrets", user1Token, mapOf(
            "name" to "cross-user-test",
            "value" to "test-value",
            "userId" to "user1"
        ))
        ...
        if (createResponse.statusCode() == 201) {
            ...
            val accessResponse = makeRequestWithToken("GET", 
                "$vaultServiceUrl/api/v1/secrets/$secretId", user2Token)
            assertTrue(accessResponse.statusCode() in listOf(403, 404),
                "Cross-user access should be prevented")
            
            // Cleanup
            makeRequestWithToken("DELETE", "$vaultServiceUrl/api/v1/secrets/$secretId", user1Token)
        }
        
        println("✅ User permissions are enforced correctly")
    }
    
    @Test
    fun `input validation prevents injection attacks`() = runTest {
    ...
    @Test
    fun `sensitive data is properly encrypted`() = runTest {
        val validToken = authenticateUser("test-user", "test-password")
        ...
        println("✅ Sensitive data is properly encrypted")
    }
    
    @Test
    fun `API rate limiting prevents abuse`() = runTest {
        val validToken = authenticateUser("test-user", "test-password")
        ...
        println("✅ API rate limiting prevents abuse")
    }
    
    @Test
    fun `audit logging captures all security events`() = runTest {
        ...
        // Check if audit logs are available (if implemented)
        val auditLogsResponse = makeRequestWithToken("GET", 
            "$apiGatewayUrl/api/v1/audit-logs?userId=$testUserId", validToken)
        
        if (auditLogsResponse.statusCode() == 200) {
            val logsResult = json.decodeFromString<Map<String, Any>>(auditLogsResponse.body())
            val logs = logsResult["data"] as? List<Map<String, Any>>
            
            if (logs != null && logs.isNotEmpty()) {
                // Verify that security events are logged
                val securityEvents = logs.filter { 
                    val eventType = (it["eventType"] as? String) ?: ""
                    eventType.contains("auth") || eventType.contains("access") || eventType.contains("security")
                }
                
                assertTrue(securityEvents.isNotEmpty(), 
                    "Audit logs should contain security-related events")
            }
        }
        
        println("✅ Audit logging captures all security events")
    }
